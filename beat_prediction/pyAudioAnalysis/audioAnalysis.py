...
def beatExtractionWrapper2(wavFileName, plot):
    if not os.path.isfile(wavFileName):
        raise Exception("Input audio file not found!")
...
def beatExtractionWrapper(wavFileName, plot):
    ...
    if not os.path.isfile(wavFileName):
        raise Exception("Input audio file not found!")
    [Fs, x] = audioBasicIO.readAudioFile(wavFileName)
    F = aF.stFeatureExtraction(x, Fs, winSize * Fs, winSize * Fs)
    stFeatures = aF.beatExtraction(F, winSize, plot)
    #print "Beat: {0:d} bpm ".format(int(BPM))
    #print "Ratio: {0:.2f} ".format(ratio)
    return stFeatures

def beatExtractionWrapperBoolean(wavFileName, plot=False):
    winSize = 0.050
    if not os.path.isfile(wavFileName):
        raise Exception("Input audio file not found!")
    [Fs, x] = audioBasicIO.readAudioFile(wavFileName)
    F = aF.stFeatureExtraction(x, Fs, winSize * Fs, winSize * Fs)
    pos1 = aF.beatExtractionBoolean(F, winSize, plot)
    #print "Beat: {0:d} bpm ".format(int(BPM))
    #print "Ratio: {0:.2f} ".format(ratio)
    return pos1
